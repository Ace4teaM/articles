...
-- Maintenant on va demander a SQL Server de réparer la base de données avec perte de données possible

--1. définit la base en mode urgence
ALTER DATABASE test_log SET EMERGENCY

--2. la base doit être en mode single user 
ALTER DATABASE test_log set SINGLE_USER

--3. si la premiere technique n'a fonctionnée on essai avec perte de données authorisée
dbcc checkdb('test_log',REPAIR_ALLOW_DATA_LOSS) WITH NO_INFOMSGS, ALL_ERRORMSGS

--4. rétablie la base en mode en-ligne
ALTER DATABASE test_log SET ONLINE

--5. permet a nouveau les connexions utilisateurs
